...
@interface XGDLMMFnishViewController ()

@end

@implementation XGDLMMFnishViewController
...
- (IBAction)subimAction:(UIButton *)sender {
    ...
    if ([self check]){
        ...
        [manager POST:[NSString stringWithFormat:@"%@?token=%@", kResetPwd, StandardUserDefautsGet(@"token")] parameters:@{@"type":@"pwd",@"pwd":_passWordText1.text,@"phone":_phoneNum} success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
